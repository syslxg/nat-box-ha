...
subnet_pub_a=`retry "ec2 create-subnet --cidr-block 10.8.0.0/24 --vpc-id $vpc --availability-zone $BOSH_VPC_PRIMARY_AZ" | jq .Subnet.SubnetId -r` 
echo $subnet_pub_a

subnet_pub_b=`retry "ec2 create-subnet --cidr-block 10.8.1.0/24 --vpc-id $vpc --availability-zone $BOSH_VPC_SECONDARY_AZ" | jq .Subnet.SubnetId -r`
echo $subnet_pub_b

subnet_prv_a=`retry "ec2 create-subnet --cidr-block 10.8.2.0/24 --vpc-id $vpc --availability-zone $BOSH_VPC_PRIMARY_AZ" | jq .Subnet.SubnetId -r`
echo $subnet_prv_a

subnet_prv_b=`retry "ec2 create-subnet --cidr-block 10.8.3.0/24 --vpc-id $vpc --availability-zone $BOSH_VPC_SECONDARY_AZ" | jq .Subnet.SubnetId -r`
echo $subnet_prv_b
...
ec2 create-tags --resources $subnet_pub_b --tags Key=Name,Value=Public-b
...
nat_b=`ec2 run-instances --image-id    $nat_ami --count 1 --instance-type t1.micro --key-name bosh --security-group-ids $sg --subnet-id $subnet_pub_b \
    --private-ip-address 10.8.1.5 | jq .Instances[0].InstanceId -r`
...
server_b=`ec2 run-instances --image-id $server_ami --count 1 --instance-type t1.micro --key-name bosh --security-group-ids $sg --subnet-id $subnet_prv_b \
    --private-ip-address 10.8.3.9 | jq .Instances[0].InstanceId -r`
...
ec2 create-tags --resources $server_b      --tags Key=Name,Value=Server_B
